// this script uses variables that have been defined elsewhere
// var filter = "${resub}sysparm_query}"
// var table = "${ref}";
// <TBODY id="gcond_filters"></TBODY>
...
// query operators in DBQuery
// 	public static final String[] SIMPLE_OPERATORS = { "!=", ">=", "<=", "=", ">", "<" };
//	public static final String[] OPERATORS =   { "IN", "!=", ">=", "<=", "=", ">", "<", "NOT IN", "ORDERBYDESC", "ORDERBY" };
//	public static final String QUERY_TERM_SEPERATOR = "^";
var operators = [
	"BETWEEN",
	"!=",
	">=",
...
//list all operators involved in field to field comparison
...
opersNS.opdef = {
    ...
    'eq'  : [ '=',          'is'                 ],
    //dynamic reference operands for SNExpress, e.g., replacing "javascript:gs.getUserID()" with "Me"
...
opersNS.compile = function(ops_input, opsdef) {
    // first pass resolves opdef array definition...
    ...
    // second pass resolves field type references...
    for (var fieldType in ops_input) {
        var proto = ops_input[fieldType];
...
// The map defined below is a prototype that is transformed by the opersNS.compile() invocation immediately following.  The 
// transformation creates an array of two-element operator definition arrays (see opersNF.opdef, above).  Each "word" in the
// strings below becomes one two-element operator definition.  The patterns "=xxx" are resolved on a second pass, where they
// are transformed into a reference to property xxx in this same map.

// *DO NOT* use ':' in the data type as this would break many many things...
// It is currently use for date_part type but can be extended to different complex types
var sysopers = {
    ...
    'email_script'        : '=string',
    'field_name'          : '=string',
    'glide_duration'      : 'eq,nteq,em,ntem,lt,gt,lteq,gteq,any,fvc',
    'glide_list'          : 'lk,ntlk,em,ntem,fvc,fvf,fvt',
    'GUID'                : '=string',
    'html'                : 'lk,ntlk,em,ntem,any,fvc,fvf,fvt',
    'html_script'         : '=string',
    'integer'             : 'eq,nteq,em,ntem,lt,gt,lteq,gteq,any,fvc,fvf,fvt,saas,nsas,gtfd,ltfd,gteqfd,lteqfd',
    'integer_choice'      : 'eq,nteq,inna,ntin,em,ntem,lt,gt,lteq,gteq,any,fvc,fvf,fvt,saas,nsas',
    'journal'             : 'fvc',
    ...
    'string'              : 'stwt,enwt,lk,ntlk,eq,nteq,em,ntem,mpat,mreg,any,inna,es,fvc,fvf,fvt,saas,nsas',
...
// extended operators that can be used in conditions, but not list filters...
var extopers = {};
...
var calNS = {};
// table for sysvalues['calendar'] -- information for creating the proper encoded query for human-readable relative time queries
// column 0: human readable label
// column 1: encoding for the "before" condition
// column 2: encoding for the "on" condition
// column 3: encoding for the "after" condition
// encoding is one of the following patterns:
//     $it
//         $ = [ for "start", ] for "end"
//         i = d for days, m for months, q for quarters, h for hours, n for minutes
//         t = quantity
//
//     $%x
//         $ = [ for "start", ] for "end"
//         % = < for "last", = for "this", > for "next"
//         x = w for week, m for month, q for quarter, y for year
// see the _build() function in GlideFilterDate.js for usage, position passed to _addDateChoices() refers to the column number from this table
calNS.protoVal = [
             'Today: [0d   ]0d  ]0d  ',
         'Yesterday: [1d   ]1d  ]1d  ',
          'Tomorrow: [-1d  ]-1d ]-1d ',
...
// PRB582945: Changed from 'Next 2 quarters: [-1q  ]-2q ]-1q '
...
calNS.compileCal = function compileCal() {
   ...
   //for ISMORETHAN AND ISSLESSTHAN and SAMEAS
...
// "name=jimmy^opened_by=johnny^opened@between@=10/10/05~10/15/05";
...
// can't sort on journal entries, arrays
...
// Run a filter removing the groupBy and adding the group value as a filter condition
function displayGroupList(element, name, groupField, groupValue, groupQuery) {
  ...
  // break the filter up and then put back together
  // ignoring the GROUPBY element
  var gfilters = query.split("^");
  var filter = '';
  //
  // Get all the query conditions
  //
  ...
  //
  // tack on the group value
  //
  if (filter.length > 0 )
     filter += "^";
  filter += groupQuery; //groupField+'='+groupValue;
  //
  // Get all the sort specifications
  //
  for (var i = 0; i < gfilters.length; i++) {
     var idx = gfilters[i].indexOf('GROUPBY');
     if (idx > -1)
...
//
// note that the order is tacked on at the end and 
// orderby is built in a global variable
function getFilter(name, doEscape, fDiv) {
	...
	// End Query (EQ) indicator
	if (fullFilter.length > 0)
		fullFilter += "^EQ";

	// add the global OrderBy variable
	// FIXME: This should NOT be in a global variable
	fullFilter += orderBy;
	filter = fullFilter;
	if (doEscape)
	...
	function addOrderBy() {
		...
		for (var i = 0; i < fQueries.length; i++) {
			...
			// this may alter the global variable orderBy
...
function getTRFilter(tr, field, oper) {
	...
	if (tr.handler) {
		...
		// call the handler to get the value information
		val = tr.handler.getValues();
		// FIXME: The SINCE operator should not have to be special cased here -
		// just support SINCE as an operator
		if (oper == 'SINCE')
			oper = '>';
	}
...
// still called from a variety of places other than element_conditions
 ...
 * 
 * @param s			: 	drop down
 * @param target	:	full field (and function) name i.e. incident.problem_id.open_at.day()
 * @param fValue	:	field value i.e. open_at.day()
...
function addFirstLevelFields(s, target, fValue, filterMethod, fieldName, filter) {
	...
	for (var i = 0; i < items.length; i++) {
		...
		if (!elementDef || elementDef.getType() != "glide_var") {
			// don't allow selection of a reference to a var__ table
			savedItems[label] = t;
...
// change the operators and the input definition based on the 
// data type of the field selected
function buildFieldsPerType(tableName, tr, descriptorName, fOper, fValue, includeExtended, tableNameFull, filterClass) {
   ...
   if (type == "float")
	  ...
	  type = "calendar";
	  // FIXME: Remove these special cases - they should be supported operators
   ...
   if ((type == "boolean") || (type == 'string_boolean')) {
      ...
      tr.handler.setChoices(sysvalues[type]);
   	  ...
   	  tr.handler.setOriginalTable(tableNameFull); //hang on to the original table for the right operand 
...
function shouldShowDynamicReferenceOperator(type, elementDef) {
	...
	//if we have no dynamic reference operands, do nothing
	if (typeof g_dynamic_filter_options == "undefined" || g_dynamic_filter_options == '')
		return false;
		
	//see if we have any dynamic reference operands for this table
...
function addTextInput(td, dValue, type) {
    ...
    if (isMSIE) {
		...
		// IE had a weird bug where it forced a 1px padding on top
		// when the input text box was valign'd top... let's defeat
		// this by setting the margin to -1 prior to IE8
		if (isMSIE7 || isMSIE6)
...
function addOperators(td, type, dValue, isChoice, includeExtended, showDynamicReferenceOperator, filterClass) {
  ...
  for(var ii=0; ii < opers.length; ii++) {
  	...
  	//Compare a list of known extended operators against list of extended operators specified
  	//to be included for this GlideFilter
  	if (extopers[opInfo[0]] && !includeExtended[opInfo[0]])
  	    continue;

	//dynamic reference operands for SNExpress, e.g., replacing "javascript:gs.getUserID()" with "Me"
	//TODO: enhance performance for SNEnterprise instances by caching on server/client
...
function addChoices(response, args) {
    ...
    // TODO:  Need to add empty option... int = 0, string = ""
...
//////////////////////////////////////////////////////////////////////////////
// column request and response stuff
...
// called from element_conditions and element_variable_conditions.xml
// largely copied code betwen the two of them
...
// decode the string in the global variable "filter"
...
function slushbucketDOMcheck() {
	...
	if(epObject= gel("ep")) {
		...
		// for loop used as simple alternative to getElementsByClassName
...
function listDOMcheck(el) {
	// find a parent DIV that has an id that ends with MAIN_LAYER
	while (el) {
...
// all parameters are optional - if null or not specified, then they are ignored
